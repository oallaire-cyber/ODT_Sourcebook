...
| [#INC-02](#inc-02) | INC | GP1 `illustrates`: index vs entry | Low | Superseded | — (by INC-06, 2026-06-30) |
...
| [#INC-06](#inc-06) | INC | S1 & GP1 `illustrates` mis-pointed to indirect risks | Low | Applied | — (2026-06-30; S1→RC-05, GP1→RH-01/RA-05) |
...
### INC-02
**GP1 `illustrates`: index vs entry.** `Sev Low · Applied (2026-06-26) · Superseded (2026-06-30, [[#INC-06]])`
...
**Superseded (2026-06-30, [[#INC-06]]):** both of GP1's targets were the *indirect* ones — RH-04 a market-structure risk and RA-02 a downstream budget consequence — so the obvious launch-disruption (RH-01) and export-control (RA-05) risks went unillustrated. INC-06 re-points GP1 to **[RH-01, RA-05]**; RA-02 stays reachable indirectly (RA-05 → RA-04 → RA-02 → RC-02 → FN1), so the cross-perimeter reach this ticket secured is retained, not lost.
...
### INC-06
**S1 & GP1 `illustrates` pointed at indirect business risks.** `Sev Low · Applied (2026-06-30)`
Surfaced reviewing the SPICE-bridge coverage: the convergence/illustration links for two realistic-case families named risks that only *indirectly* fit their narratives, while the obviously-illustrated risk went unlinked. (Supersedes [[#INC-02]], which had added one of the indirect targets.)
- **BST-S1** (NOC / Ground-Segment Compromise) illustrated **RH-04** "Loss of competitive positioning vs mega-constellations" — a market-structure risk. A cyber outage that breaks SLAs materialises as **reputational harm**, so it now illustrates **RC-05** (whose definition explicitly covers "service outage"). RC-05 already carried the better edges: `INFLUENCES` RC-05 → RC-03 (INF-28) → `IMPACTS_TCO` TCO-04 (IMC-06) keeps S1 on the IPO apex, RC-05 → TCO-03 (IMC-03) and RC-05 → TPO-04 (IMP-09) carry the commercial/government-confidence thread, and INF-15 (SEC-01 → RC-05) backs the cyber→reputational cause path S1's own `caused_by` names.
- **BST-GP1** (Export-Control Tightening & Launch Disruption) illustrated **RH-04 + RA-02** — both indirect. Re-pointed to **RH-01** (Phase-3 launch schedule delay — the launch-disruption half) + **RA-05** (ITAR/export-control blocking partnerships — the export-control half), the risks GP1's own narrative names. Cross-perimeter reach to AURORA's budget is preserved indirectly: RA-05 → RA-04 (INF-31) → RA-02 (INF-47) → RC-02 → FN1; LEO path: RH-01 → RC-01 (INF-21) → TCO-04 (IMC-04).
**Applied (2026-06-30):** owner decision (a SpiceScenario may illustrate >1 Business Risk — confirmed). `_inputs/workbook.yaml`: S1 `illustrates: RC-05`, GP1 `illustrates: [RH-01, RA-05]` → regenerated `demo_seed.cypher` (new edges `ILL-BST-S1-RC-05`, `ILL-BST-GP1-RH-01`, `ILL-BST-GP1-RA-05`; stale RH-04/RA-02 illustrates edges removed; 74 spice edges). Propagated to both bestiary entries + [[Bestiary Index]], [[Annex - RIM Anchoring]] (stitched-chain example), [[Annex - AURORA-GEO Programme]] (INC-02 note), [[Annex - External Environment & Constraints]] §8, [[Crisis Exercise - S1 NOC Compromise]] apex line, and the [[ODT Risk Status & Executive Decision Brief - 2026-06-28|risk-manager brief]]. No canon figures changed (ILLUSTRATES is a structural bridge edge; all six families still provably reach TCO-04). All graph edges cited above pre-existed — only the bridge endpoints moved.
...
## Changelog
- 2026-06-30: **+INC-06 raised & Applied — S1 & GP1 `illustrates` re-pointed off indirect risks (supersedes INC-02).** SPICE-bridge review found two realistic-case families illustrating risks that only indirectly fit their narratives. **S1** RH-04 → **RC-05** (cyber outage → reputational harm); **GP1** [RH-04, RA-02] → **[RH-01, RA-05]** (launch-disruption + export-control, the two halves it actually names). AURORA budget (RA-02) stays reachable indirectly via RA-05 → RA-04 → RA-02. `workbook.yaml` → regenerated `demo_seed.cypher` (74 spice edges; new ILL-BST-S1-RC-05 / ILL-BST-GP1-RH-01 / ILL-BST-GP1-RA-05). Propagated across both bestiary entries, Bestiary Index, RIM Anchoring, AURORA + External-Environment annexes, the S1 crisis exercise, and the risk-manager brief. No canon figures changed; all 6 families still reach TCO-04.
