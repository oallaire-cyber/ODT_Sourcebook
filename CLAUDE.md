...
2. **Schema-align.** Match `schema.canonical.yaml` v3.0 (RIM's secure canonical schema; replaces the former `schema.yaml`) — node labels, fields, enums — and the entity IDs in `demo_seed.cypher`. Don't guess field names. The schema is **RIM-owned**; it changes only via a **Change Request** (see "Working with the RIM project" below) — never fork it unilaterally.
...
- `schema.canonical.yaml` — RIM schema v3.0 (entities, relationships, context nodes, SpiceScenario). The **secure canonical schema** produced by the RIM project; supersedes the former `schema.yaml`.
