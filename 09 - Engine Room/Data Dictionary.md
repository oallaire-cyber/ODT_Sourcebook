...
> [!info] How the vault's content maps to `schema.canonical.yaml` v3.0 (the RIM-produced secure canonical schema; replaces the former `schema.yaml`) and loads via `demo_seed.cypher`. Source of truth for **data**: the editable workbook → generates the Cypher (one source, never two).
...
| Section | Content |
|---|---|
| `risks` | All Risk nodes (**55 total**: 50 active/contingent RC/RH/RA/ROE/ROM/ROL/ROC/ROF/ROH/ROR/ROI/SEC/RCY + **5 historical HX-01..05**, W5 — lifecycle states `is_active:false`) |
...
| `context_nodes` | Business perimeters, scenarios, entry points, attackers, technical perimeters, sponsors, functional targets, business activities, **suppliers (8, CR-02)**, **regulators (5, CR-02)** |
...
| `relationships.context_edges` | Kill-chain, business wiring & external entities (**71 edges** — incl. CR-02: 8 `SUPPLIES`, 6 `SOURCED_FROM`, 9 `GOVERNS`) |
...
| Supplier (CR-02) | `ContextNode{supplier}` | name, `tier` {Tier 1/2/3/Internal}, `criticality` {Critical/Important/Standard}, `single_source` (bool), `country`, `lead_time`; `SUPPLIES` business_activity, `SOURCED_FROM`←risk |
| Regulator / regime (CR-02) | `ContextNode{regulator}` | name, `kind` {regulator/export_control/accreditation/partnership}, `authority`, `standard`, `jurisdiction`; `GOVERNS` risk |

## Edges
`INFLUENCES` (level1/2/3; weak→critical) · `MITIGATES` · `IMPACTS_TPO` / `IMPACTS_TCO` · `CONTRIBUTES_TO` · accountability: `BEARS` `STEWARDS` · SPICE: `ILLUSTRATES` `CAUSED_BY` `ASSESSED_AGAINST` `OCCURS_AT` `ADDRESSES` `FULFILS` (now `Mitigation→objective`) `MITIGATED_BY` (CR-01, was `MITIGATED_BY_SPICE`) · **production chain (W4): `USES` (perimeter→activity) `PRODUCES` (activity→target) `HOSTED_ON` (target→asset) `CONCERNS` (risk→asset)** · cyber path: `SEEKS` `EXPLOITS` `COMPROMISES` `EXPOSES` `AFFECTS` `TARGETS` · **external entities (CR-02): `SUPPLIES` (supplier→business_activity) `SOURCED_FROM` (risk→supplier) `GOVERNS` (regulator→risk)**.

### Supply-chain production graph (W4) + external entities (CR-02)
The supply chain is a graph, not prose (see [[Annex - Supply Chain & Production]]). Since **CR-02 landed** (RIM aligned `schema.canonical.yaml`, 2026-06-29), **suppliers and regulators are first-class context nodes**, no longer only risk attributes:
- **8 `supplier` nodes** (SUP-01..08: Teledyne, Saft, Northrop Grumman, Spirit AeroSystems, Moog, Orbital ATK, SpaceX, Rocket Lab) `SUPPLIES`→`business_activity` (components → **BA-PROC**; launch → **BA-LAUNCH**), unlocking supplier **blast-radius** into the chain `(:supplier)-[:SUPPLIES]->(:business_activity)-[:PRODUCES]->(:functional_target)-[:HOSTED_ON]->(:technical_perimeter)`. `SOURCED_FROM` ties the 5 external supply-chain risks + incident HX-01 back to their supplier (ROM-04 is the *internal* AIT clean room = BA-MFG itself, so it has no supplier node, per CR-02 §8). Filter `single_source: true` → the Teledyne (RF) + Saft (battery) choke points.
- **5 `regulator` nodes** (REG-01..05: FCC, ITU, US DDTC/ITAR, DoD accreditation, ESA/NASA) `GOVERNS`→the 6 `regulatory_compliance` risks + incident HX-05 — unlocking "everything the FCC governs" (ROR-01, RC-04, HX-05) and "ODT's whole ITAR surface" (RA-05 + ROH-02).
- The `supply_chain` / `regulatory_compliance` **risk subtypes stay, denormalised** (zero migration, per CR-02 §3 Q3): `supplier_tier`/`criticality_class`/`single_source` on ROM-01/02/03/04, ROL-01, SEC-02 and `regulatory_body`/`applicable_standard`/`licence_stage` on the 6 regulatory risks — the node duplicates them intentionally.
- The **production chain** itself is unchanged: `BP-LEO`-`USES`→`business_activity`(5)-`PRODUCES`→`functional_target`(5)-`HOSTED_ON`→`technical_perimeter`; assets **TP-AIT** (clean-room integration line), **TP-NOC** (Denver/Dublin ops & service). All CR-02 nodes/edges are **context-only** (no exposure/magnitude math).
...
> - ~~**No company history / past incidents**~~ **Resolved (2026-06-29, W5).** Added **5 historical risks** (HX-01..05) exercising the lifecycle states — `Closed` (×2), `Archived`, `Watching` (accept-and-watch with `trigger_condition`/`acceptance_date`/`acceptance_owner`), `Suppressed` — all `is_active:false` (excluded from live exposure, retained for audit). Generator emits the lifecycle fields + a "Historical Incidents" risk group; narrative in [[ODT Company History & Incident Log]]. Operational-texture (no major breach / on-schedule delivery preserved); THE NUMBER stays forward-looking.
> - ~~**External constraints not modelled**~~ **Resolved (2026-06-29, W3).** `regulatory_compliance` risk subtype (`regulatory_body`/`applicable_standard`/`licence_stage`) populated on **6 risks** (RC-04, RH-05, RH-07, RA-05, ROH-02, ROR-01 — FCC/ITU/ITAR/DoD) + [[Annex - External Environment & Constraints]] (the 6-layer perimeter: spectrum, export control, accreditation, ESA/NASA terms, launch market, capital-market window). ~~*Suppliers (W4) and regulators (W3) are modelled as risk attributes + annexes for now...*~~ **Promoted to first-class nodes — [[CR-02 - External Entities (Supplier & Regulatory)|CR-02]] CONVERGED & APPLIED (2026-06-29):** RIM aligned `schema.canonical.yaml` (`supplier`/`regulator` context nodes + `SUPPLIES`/`GOVERNS`/`SOURCED_FROM` edges); ODT regenerated — 8 supplier + 5 regulator nodes + 23 edges. Subtypes kept denormalised. (Launch-market / capital-market window stay narrative until a future `market_factor` type.)
> - ~~**Cyber kill-chain + technical perimeters (ENH-04) + usable λ/magnitude**~~ **Resolved (2026-06-29, W6).** S1 NOC kill-chain wired (`SPN-02`→`ATK-02`→`EP-02`→`COMPROMISES` `TP-IDP`/`TP-NMS`/`TP-NOC`→`SEEKS` `FT-CTRL`); **4 new technical perimeters close ENH-04** (TP-IDP, TP-NMS, TP-GW, TP-BILL; 7 perimeters total). **λ** (`annual_probability`) now on every risk via the `likelihood_to_lambda` Moderate-band fallback in `generate_seed.py` (explicit value wins); **magnitude** Tier-2 fallback (`magnitude_point_estimate`/`low`/`high`) on the 7 SPICE-uncovered Business Risks. Calibration rules in [[Canon & Figures Register]] ("Frequency & magnitude calibration"). *(Feeds the loss-exceedance curve; closes ENH-04.)*
